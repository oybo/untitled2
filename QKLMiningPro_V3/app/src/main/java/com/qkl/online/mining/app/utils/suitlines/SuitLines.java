...
package com.qkl.online.mining.app.utils.suitlines;
...
public class SuitLines extends View {
    ...
    public SuitLines(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        xGridPaint.setStrokeWidth(1f);
        xGridPaint.setColor(Color.parseColor("#30DEDEDE"));
        yGridPaint.setStrokeWidth(1f);
        yGridPaint.setColor(Color.parseColor("#30DEDEDE"));
    }
     ...
     */
    private int[] defaultLineColor = {Color.WHITE, Color.WHITE, Color.WHITE};
    private int hintColor = Color.RED;
    /**
     * xy轴文字颜色和大小
     */
    private int defaultXyColor = Color.WHITE;
    ...
    private Paint xGridPaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    private Paint yGridPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private void drawX(Canvas canvas, int startIndex, int endIndex) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            ...
            if (showXGrid) {
                canvas.drawLine(datas.get(0).get(i).getXY().x, xArea.top,
                        datas.get(0).get(i).getXY().x, 0, xGridPaint);
            }
        }
    }
    ...
    private void drawY(Canvas canvas) {
        if (yAreaBuffer == null) {
            ...
            for (int i = 0; i < countOfY; i++) {
                ...
                yCanvas.drawText(new DecimalFormat("##0.0").format(extY) + " $", yRect.right - basePadding, y, xyPaint);
        ...
        if (yGridBuffer == null) {
            ...
            for (int i = 0; i < countOfY; i++) {
                ...
                if (i == 0) {
                    ...
                    yAxis = yRect.bottom - yRect.height() / (countOfY - 1) * i + Util.getTextHeight(yGridPaint) / 2;
                }
                yCanvas.drawLine(0, yAxis, yCanvas.getWidth(), yAxis, yGridPaint);
            }
            if (minAndMaxOfY[0] != 0 && minAndMaxOfY[1] != 0) {
                float y = zeroAxisValue - yArea.top;
                yCanvas.drawLine(0, y, yCanvas.getWidth(), y, yGridPaint);
            }
        }
        if (showYGrid) {
...
//        minAndMaxOfY[1] = 1f;
    ...
    private void calcAreas() {
        ...
        yArea = new RectF(validArea.left, validArea.top,
                validArea.left + maxWidth + basePadding * 2,
        ...
        hintArea = new RectF(linesArea.right - linesArea.right / 4, linesArea.top,
                linesArea.right, linesArea.top + linesArea.height() / 2);
    }
